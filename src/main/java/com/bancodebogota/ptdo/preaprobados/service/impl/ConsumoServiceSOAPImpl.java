 ...
 * <p>
 * Class para el consumo del servicio SOAP implementa {ConsumoServiceSOAP}
 * 
 * @author Stiven Diaz
 * 
 */
...
public class ConsumoServiceSOAPImpl implements ConsumoServiceSOAP {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.bancodebogota.ptdo.preaprobados.service.ConsumoServiceSOAP#
	 * getCampPotentialSale(java.lang.String, java.lang.String, java.lang.String,
	 * java.lang.String)
	 */
	public GetCampPotentialSaleResponse getCampPotentialSale(String tipoDocumento, String numeroDocumento,
			String usuario, String endpoint) {
		if ("".equals(endpoint))
			endpoint = consumoServiceREST.getParametro(pEndpointBUSPreAprobados);
		// String endpoint =
		// "http://10.85.88.126:15090/customers/ProductCampPotentialSaleInquiry";//
		// externo desarrollo
		// String endpoint =
		// "http://10.87.52.23:10088/customers/ProductCampPotentialSaleInquiry";//
		// interno desarrollo
		...
		try {
			...
			log.info("Ingreso a SoapFaultClientException");			
			SoapFaultClientException spe = (SoapFaultClientException) e;
			SoapFaultDetail soapFaultDetail = spe.getSoapFault().getFaultDetail(); // <soapFaultDetail> node
			
			try {
				SoapFaultDetailElement detailElementChild = soapFaultDetail.getDetailEntries().next();				
				Source detailSource = detailElementChild.getSource();
				Object detail = soapConnector.getWebServiceTemplate().getUnmarshaller().unmarshal(detailSource);				
				JAXBElement<GeneralExceptionType> source = (JAXBElement<GeneralExceptionType>) detail;
				System.out.println("Text::" + source.getValue().getStatus().getServerStatusDesc());				
			} catch (Exception ex) {
				//ex.printStackTrace();
			}
		}
			...
			if (excepcion instanceof SOAPFaultException) {
				...
				try {
					statusCode = objectUtils.obtenerValorPorExpresion(spe.getFault(),
							"//*/*[local-name()='GeneralException']/*[local-name()='Status']/*[local-name()='StatusCode']");
					if (statusCode.length() == 0 || statusDesc.length() == 0) {
						throw spe;
					}
					faultObject = objectUtils.obtenerValorPorExpresion(spe.getFault(),
							"//*/*[local-name()='GeneralException']");
			...
			/*
			 * log.warning(statusCode); log.warning(statusDesc);
